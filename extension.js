...
var VitalsMenuButton = GObject.registerClass(
  ...
  class VitalsMenuButton extends PanelMenu.Button {
    _init(extensionObject) {
      ...
      this._sensorIcons = {
        temperature: {
          icon: "temperature-symbolic.svg",
          "icon-c": "water-droplet-symbolic.svg",
        },
      ...
      this._addSettingChangedSignal(
        "update-time",
        this._updateTimeSettingChanged.bind(this),
      );
      this._addSettingChangedSignal(
        "position-in-panel",
      ...
      this._addSettingChangedSignal(
        "icon-style",
        this._iconStyleChanged.bind(this),
      );
      ...
      for (let sensor in this._sensorIcons)
        this._addSettingChangedSignal(
          "show-" + sensor,
          this._showHideSensorsChanged.bind(this),
        );
    ...
    _initializeMenu() {
      ...
      let monitorButton = this._createRoundButton(
      ...
      this._menuStateChangeId = this.menu.connect(
        "open-state-changed",
        (self, isMenuOpen) => {
          if (isMenuOpen) {
            // make sure timer fires at next full interval
    ...
    _initializeMenuGroup(
      ...
      if (!this._groups[groupName].status) {
        ...
        this._groups[groupName].actor.insert_child_at_index(
          this._groups[groupName].status,
          4,
        );
    ...
    _saveHotSensors(hotSensors) {
      ...
      this._settings.set_strv(
        "hot-sensors",
        hotSensors.filter(function (item, pos) {
    ...
    _createHotItem(key, value, gicon) {
      ...
      if (
        !this._settings.get_boolean("hide-icons") ||
        key == "_default_icon_"
      ) {
    ...
    _showHideSensorsChanged(self, sensor) {
      ...
      if (sensorName === "gpu") {
        for (let i = 1; i <= this._numGpus; i++)
          this._groups[sensorName + "#" + i].visible =
            this._settings.get_boolean(sensor);
      } else
    ...
    _redrawDetailsMenuIcons() {
      ...
      for (const sensor in this._sensorIcons) {
        if (sensor == "gpu") continue;
        this._groups[sensor].icon.gicon = Gio.icon_new_for_string(
          this._sensorIconPath(sensor),
        );
      }

      // gpu's are indexed differently, handle them here
      const gpuKeys = Object.keys(this._groups).filter((key) =>
        key.startsWith("gpu#"),
      );
      gpuKeys.forEach((gpuKey) => {
        this._groups[gpuKey].icon.gicon = Gio.icon_new_for_string(
          this._sensorIconPath("gpu"),
        );
      });
    }
    ...
    _appendMenuItem(sensor, key) {
      ...
      if (type === "fan" && key.includes("aio")) {
        icon = "icon-pump";
      }
      if (type === "temperature" && key.includes("coolant")) {
        icon = "icon-c";
      }
      ...
      if (this._settings.get_boolean("alphabetize")) {
        ...
        for (i = 0; i < menuItems.length; i++)
          ...
          if (
            menuItems[i].label.localeCompare(item.label, undefined, {
              numeric: true,
              sensitivity: "base",
            }) > 0
          )
    ...
    _defaultIcon(key) {
      ...
      if (
        type == "default" ||
        (!(type in this._sensorIcons) && !type.startsWith("gpu"))
      ) {
        ...
        icon.gicon = Gio.icon_new_for_string(
          this._sensorIconPath(type, iconObj),
        );
      }
    ...
    _sensorIconPath(sensor, icon = "icon") {
      ...
      if (
        ...
        icon === "icon" &&
        this._lastHotSensorKey &&
        this._lastHotSensorKey.toLowerCase().includes("coolant")
      ) {
        icon = "icon-c";
      }
      if (
        ...
        icon === "icon" &&
        this._lastHotSensorKey &&
        this._lastHotSensorKey.toLowerCase().includes("aio")
      ) {
        icon = "icon-pump";
      }
      ...
      return (
        ...
        this._sensorsIconPathPrefix[iconPathPrefixIndex] +
        this._sensorIcons[sensorKey][icon]
      );
    }
    ...
    _querySensors() {
      ...
      this._sensors.query((label, value, type, format) => {
        ...
        let key =
          "_" + typeKey + "_" + label.replace(" ", "_").toLowerCase() + "_";
        ...
        let items = this._values.returnIfDifferent(
          dwell,
          label,
          value,
          type,
        ...
        for (let item of items) {
          if (
            item.type.startsWith("network-") &&
            item.type.length == 10 &&
            item.type != "network-rx" &&
            item.type != "network-tx"
          ) {
            // Geo / flags: stable key (no country in key); type stays network-<cc> for icon-us etc.
            const stem = item.type.slice("network-".length);
            let flagGIcon = Gio.icon_new_for_string(
              this._sensorIconPath("network", "icon-" + stem),
            );
            if (
              this._hotItems[item.key] &&
              !this._settings.get_boolean("hide-icons")
            ) {
              // change icon in menu bar
              let icon = this._hotItems[item.key].get_first_child();
              if (icon instanceof St.Icon) icon.gicon = flagGIcon;
            }
